...
final.data <- pa.nests.prop %>%
  dplyr::select(nestd_v, bandid, wmu_v, avrgmxv, grssfrb, percwdy,
                case, uniqueid, Developed, Barren, Forested,
                Agriculture,geometry)  %>%
  mutate(Developed.Z= scale(Developed)[,1])%>%
  mutate(Forested.Z= scale(Forested)[,1])%>%
...
#Distance from Road + Visual Obstruction
fit2=clogit(case~ NEARDIST + Visual.Obstruction + 
         strata(nestd_v), 
       data = final.data)

#Visual Obstruction
fit3=clogit(case~ Visual.Obstruction + 
         strata(nestd_v), 
       data = final.data)

#Developed + Visual Obstruction
fit4=clogit(case~ Developed.Z + Visual.Obstruction +
            strata(nestd_v), 
            data = final.data)
#Developed
fit5=clogit(case~ Developed.Z + 
              strata(nestd_v), 
            data = final.data)

#Barren + Visual Obstruction
fit6=clogit(case~ Barren.Z + Visual.Obstruction +
              strata(nestd_v), 
            data = final.data)

#Barren + Distance from Road
fit7=clogit(case~ Barren.Z + NEARDIST +
              strata(nestd_v), 
            data = final.data)

#Barren 
fit8=clogit(case~ Barren.Z +
              strata(nestd_v), 
            data = final.data)

#Forest 
fit9=clogit(case~ Forested.Z +
            strata(nestd_v), 
            data = final.data)

#Forest + Distance from Road
fit10=clogit(case~ Forested.Z + NEARDIST +
            strata(nestd_v), 
            data = final.data)

#Forest + Visual Obstruction
fit11=clogit(case~ Forested.Z + Visual.Obstruction +
               strata(nestd_v), 
             data = final.data)

#Agriculture
fit12=clogit(case~ Agriculture.Z +
               strata(nestd_v), 
             data = final.data)

#Agriculture + Visual Obstruction
fit13=clogit(case~ Agriculture.Z + Visual.Obstruction +
               strata(nestd_v), 
             data = final.data)

#Agriculture + Distance from Road
fit14=clogit(case~ Agriculture.Z + NEARDIST +
               strata(nestd_v), 
             data = final.data)

#GrassForb
fit15=clogit(case~ GrassForb +
               strata(nestd_v), 
             data = final.data)

#Percent Woody
fit16=clogit(case~ PercentWoody +
               strata(nestd_v), 
             data = final.data)

#Check AIC values
AIC(fit1,fit2,fit3,fit4,fit5,fit6, fit7, fit8, fit9, fit10, fit11, 
