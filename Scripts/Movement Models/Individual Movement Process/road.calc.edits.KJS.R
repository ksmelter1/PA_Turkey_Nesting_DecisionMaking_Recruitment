...
############################################
## Distance t0 Primary Road Calculations ##
############################################
...
min.road.dist<-vector(mode = "list", length = length( random.steps.list))
...
for (i in 1: length(random.steps.list)) {
    ...
    mean_buffer.prim = st_buffer(mean.coord.sf.prim, 10000)
...
#' Create an empty vector to store minimum distances from the nearest road
min.road.sec<-vector(mode = "list", length = length( random.steps.list))
...
for (i in 1: length(random.steps.list)) {
  ...
  #' Buffer the mean.coord by 10,000 meters
  mean_buffer.sec = st_buffer(mean.coord.sf.sec, 10000)
