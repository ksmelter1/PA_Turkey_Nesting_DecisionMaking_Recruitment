...
options(width=150)
dat <-  read.csv("Data Management/Csvs/Misc/d_otter.csv")
str(dat)
...
test.mod <- "  
 model{
  for (i in 1:I){
   Loc[i]~dpois(lambda[i])
   log(lambda[i])<-STAU1[i]*beta.STAU1+REST1[i]*beta.REST1+Sohlenbrei[i]*beta.Sohl+Breaks_Dis[i]*beta.Breaks+alpha[str_ID[i]]
  }
  
  #Priors
  beta.STAU1~dnorm(0,0.0001) #Reservoir
  beta.REST1~dnorm(0,0.0001) #Residual Water
  beta.Sohl~dnorm(0,0.0001)  #River width
  beta.Breaks~dnorm(0,0.0001) #Step length
...
jags_data <- list(
  ...
  STAU1 = dat$STAU1,
  REST1 = dat$REST1,
  Sohlenbrei = c(dat$Sohlenbrei),
  Breaks_Dis = c(dat$Breaks_Dis),
  str_ID = dat$str_ID,
  I=nrow(dat),
  K=length(unique(dat$str_ID))
  )
...
jags_samples <- coda.samples(jags_model, c("beta.STAU1","beta.REST1","beta.Sohl","beta.Breaks"), n.iter = 10000,  n.burnin = 2000)
